...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'spooky':
				new stages.Spooky(); // Week 2
			case 'philly':
				new stages.Philly(); // Week 3
			case 'limo':
				new stages.Limo(); // Week 4
			case 'mall':
				new stages.Mall(); // Week 5 - Cocoa, Eggnog
			case 'mallEvil':
				new stages.MallEvil(); // Week 5 - Winter Horrorland
			case 'school':
				new stages.School(); // Week 6 - Senpai, Roses
			case 'schoolEvil':
				new stages.SchoolEvil(); // Week 6 - Thorns
			case 'tank':
				new stages.Tank(); // Week 7 - Ugh, Guns, Stress
			case 'phillyStreets':
				new stages.PhillyStreets(); // Weekend 1 - Darnell, Lit Up, 2Hot
			case 'phillyBlazin':
				new stages.PhillyBlazin(); // Weekend 1 - Blazin
			case 'phillyStreetsBF':
				new stages.PhillyStreetsBF(); // Darnell (BF Mix)
		}

		if (Paths.formatToSongPath(SONG.song) == 'stress')
			GameOverSubstate.characterName = 'bf-holding-gf-dead';

		if (Note.globalRgbShaders.length > 0)
		...
		if (!opponentDrain
			&& !Math.isNaN((opponentChart ? boyfriend : dad).drainAmount)
			&& (opponentChart ? boyfriend : dad).drainFloor != 0)
			healthDrainAmount = opponentChart ? boyfriend.drainAmount : dad.drainAmount;
		if (!opponentDrain
			&& !Math.isNaN((opponentChart ? boyfriend : dad).drainFloor)
			&& (opponentChart ? boyfriend : dad).drainFloor != 0)
			healthDrainFloor = opponentChart ? boyfriend.drainFloor : dad.drainFloor;
		...
		if (ClientPrefs.timeBarStyle != 'Dave Engine')
			add(timeBar);
		...
		switch (ClientPrefs.timeBarStyle)
		{
			...
			case 'Doki Doki+':
				...
				timeBarBG.screenCenter(X);
				timeBar.createGradientBar([FlxColor.TRANSPARENT], [
		...
		if (notes.members[0] != null)
			firstNoteStrumTime = notes.members[0].strumTime;
		...
		add(camFollowPos);
		if (!ClientPrefs.charsAndBG)
			FlxG.camera.zoom = 100; // zoom it in very big to avoid high RAM usage!!
		if (ClientPrefs.charsAndBG)
		{
		...
		if (ClientPrefs.scoreStyle == 'Tails Gets Trolled V4')
			msTxt.setFormat("calibri.ttf", 20, FlxColor.WHITE, LEFT, OUTLINE, FlxColor.BLACK);
		if (ClientPrefs.scoreStyle == 'TGT V4')
			msTxt.setFormat("comic.ttf", 20, FlxColor.WHITE, LEFT, OUTLINE, FlxColor.BLACK);
		if (ClientPrefs.scoreStyle == 'Doki Doki+')
			msTxt.setFormat("Aller_rg.ttf", 20, FlxColor.WHITE, LEFT, OUTLINE, FlxColor.BLACK);
		msTxt.x = 408 + 250;
		msTxt.y = 290 - 25;
		...
		if (ClientPrefs.scoreStyle == 'Tails Gets Trolled V4')
			judgeTxt.setFormat("calibri.ttf", 20, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		if (ClientPrefs.scoreStyle == 'Dave and Bambi')
			judgeTxt.setFormat("comic.ttf", 20, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		if (ClientPrefs.scoreStyle == 'Doki Doki+')
			judgeTxt.setFormat("Aller_rg.ttf", 20, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		...
		switch (ClientPrefs.watermarkStyle)
		{
			...
			case 'JS Engine':
				if (!ClientPrefs.downScroll)
					EngineWatermark.y = FlxG.height * 0.1 - 70;
		...
		if (ClientPrefs.watermarkStyle == 'Hide' && EngineWatermark != null)
		...
		switch (style)
		{
			...
			case 'Dave Engine', 'Psych Engine', 'VS Impostor':
				...
				scoreTxt.setFormat(Paths.font((style == 'Dave Engine' ? "comic.ttf" : "vcr.ttf")), 20,
					(style != 'VS Impostor' ? FlxColor.WHITE : FlxColor.fromRGB(dadColors[0], dadColors[1], dadColors[2])), CENTER, OUTLINE, FlxColor.BLACK);
				scoreTxt.borderSize = 1.25;

			case 'Doki Doki+', 'TGT V4':
				scoreTxt.y = healthBarBG.y + 48;
				scoreTxt.setFormat(Paths.font((ClientPrefs.scoreStyle == 'TGT V4' ? "calibri.ttf" : "Aller_rg.ttf")), 20, FlxColor.WHITE, CENTER, OUTLINE,
					FlxColor.BLACK);
				scoreTxt.borderSize = 1.25;
		...
		if (ClientPrefs.scoreTxtSize > 0 && scoreTxt != null && !ClientPrefs.showcaseMode && !ClientPrefs.hideHud)
			scoreTxt.size = ClientPrefs.scoreTxtSize;
		...
		if (ClientPrefs.downScroll)
			renderedTxt.y = healthBar.y + 50;
		if (ClientPrefs.scoreStyle == 'VS Impostor')
			renderedTxt.y = healthBar.y + (ClientPrefs.downScroll ? 100 : -100);
		add(renderedTxt);
		...
		try
		{
			...
			switch (botStyle)
			{
				...
				case 'VS Impostor':
					botplayTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.fromRGB(dadColors[0], dadColors[1], dadColors[2]), CENTER, OUTLINE,
						FlxColor.BLACK);
			}
		}
		catch (e)
	...
	public function startVideo(name:String, ?library:String = null, ?callback:Void->Void = null, forMidSong:Bool = false, canSkip:Bool = true,
			loop:Bool = false, playOnLoad:Bool = true)
	{
		#if VIDEOS_ALLOWED
		inCutscene = true;
		canPause = false;
	...
	public function changeTheSettingsBitch()
	{
		...
		shouldDrainHealth = (opponentDrain || (opponentChart ? boyfriend.healthDrain : dad.healthDrain));
		if (!opponentDrain && !Math.isNaN((opponentChart ? boyfriend : dad).drainAmount))
			healthDrainAmount = opponentChart ? boyfriend.drainAmount : dad.drainAmount;
		if (!opponentDrain && !Math.isNaN((opponentChart ? boyfriend : dad).drainFloor))
			healthDrainFloor = opponentChart ? boyfriend.drainFloor : dad.drainFloor;
	}
	...
	function camPanRoutine(anim:String = 'singUP', who:String = 'bf'):Void
	{
		if (SONG.notes[curSection] != null)
		{
			...
			if (doPan && cameraSpeed > 0)
			{
				...
				{
					case 'singUP':
						moveCamTo[1] = -40 * ClientPrefs.panIntensity * 240 / fps;
					case 'singDOWN':
						moveCamTo[1] = 40 * ClientPrefs.panIntensity * 240 / fps;
					case 'singLEFT':
						moveCamTo[0] = -40 * ClientPrefs.panIntensity * 240 / fps;
					case 'singRIGHT':
						moveCamTo[0] = 40 * ClientPrefs.panIntensity * 240 / fps;
				}
			}
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				if (swagCounter > 0 && swagCounter < 4)
					createCountdownSprite(introAlts[swagCounter - 1], antialias);
						...
						if (ClientPrefs.tauntOnGo && ClientPrefs.charsAndBG)
						{
							...
							for (char in charsToHey)
							{
								if (char != null)
								{
									...
									else if (char.animOffsets.exists('singUP')
										&& (!char.animOffsets.exists('hey') || !char.animOffsets.exists('cheer')))
									{
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if (ClientPrefs.scoreStyle == 'JS Engine')
			formattedScore = formatNumber(shownScore);
		...
		npsString = showNPS ? ' $divider '
			+ (cpuControlled && ClientPrefs.botWatermark ? 'Bot ' : '')
			+ 'NPS/Max: '
			+ formattedNPS
			+ '/'
			+ formattedMaxNPS : '';
		...
		if (cpuControlled && ClientPrefs.botWatermark)
			...
			switch (ClientPrefs.scoreStyle)
			{
				case 'Kade Engine', 'Doki Doki+':
					tempScore = 'Score: ' + formattedScore + missString + (comboInfo ? ' $divider Combo: ' + formattedCombo : '') + npsString
						+ ' $divider Accuracy: ' + accuracy + ' $divider (' + fcString + ') ' + ratingName;

				case "Dave Engine":
					tempScore = 'Score: ' + formattedScore + missString + (comboInfo ? ' $divider Combo: ' + formattedCombo : '') + npsString
						+ ' $divider Accuracy: ' + accuracy + ' $divider ' + fcString;

				case "Forever Engine":
					tempScore = 'Score: ' + formattedScore + ' $divider Accuracy: $accuracy [' + fcString + ']' + missString
						+ (comboInfo ? ' $divider Combo: ' + formattedCombo : '') + npsString + ' $divider Rank: ' + ratingName;

				case "Psych Engine", "JS Engine", "TGT V4":
					tempScore = 'Score: ' + formattedScore + missString + (comboInfo ? ' $divider Combo: ' + formattedCombo : '') + npsString
						+ ' $divider Rating: ' + ratingName + (ratingName != '?' ? ' (${accuracy}) - $fcString' : '');
				...
				case 'VS Impostor':
					tempScore = 'Score: ' + formattedScore + missString + (comboInfo ? ' $divider Combo: ' + formattedCombo : '') + npsString
						+ ' $divider Accuracy: $accuracy [' + fcString + ']';
	...
	function startSong():Void
	{
		...
		if (ClientPrefs.ratingCounter && judgeCountUpdateFrame <= 4 && judgementCounter != null)
			updateRatingCounter();
		if (scoreTxtUpdateFrame <= 4 && scoreTxt != null)
			updateScore();
		...
		{
			if (cpuControlled)
				detailsText = detailsText + ' (using a bot)';
	...
	private function generateSong(dataPath:String, ?startingPoint:Float = 0):Void
	{
		...
		try
		{
			if (SONG.needsVoices)
			{
				var playerVocals = Paths.voices(curSong, diff,
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				if (songNotes[0] >= startingPoint + offsetStart)
				{
					...
					if (daStrumTime >= charChangeTimes[0])
					{
						switch (charChangeTypes[0])
						{
							case 0:
								...
								if (boyfriendToGrab != null)
									bfNoteskin = boyfriendToGrab.noteskin;
							case 1:
								var dadToGrab:Character = dadMap.get(charChangeNames[0]);

								if (dadToGrab != null)
									dadNoteskin = dadToGrab.noteskin;
						}
					...
					if (swagNote.noteskin.length > 0 && !Paths.noteSkinFramesMap.exists(swagNote.noteskin))
						inline Paths.initNote(swagNote.noteskin);
		...
		trace('\nDone! \n\nTime taken: '
			+ CoolUtil.formatTime(elapsedTime * 1000)
			+ "\nAverage NPS while loading: "
			+ Math.floor(notesLoadedRN / elapsedTime));
		notesLoadedRN = 0;
	}
		...
		for (i in 0...4)
		{
			...
			if (player < 1)
			{
				if (!ClientPrefs.opponentStrums)
					...
					targetAlpha = ClientPrefs.oppNoteAlpha;
			}
			...
			{
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {alpha: targetAlpha}, 1 / (Conductor.bpm / 240) / playbackRate,
					{ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i) / (Conductor.bpm / 240) / playbackRate});
			}
			else
			{
			...
			if (player == 1)
			{
				if (!opponentChart || opponentChart && middleScroll)
	...
	{
		if (finishTimer != null || ffmpegMode)
			return;
	...
	override public function update(elapsed:Float)
	{
		...
		if (!cpuControlled && canUseBotEnergy)
		{
			...
			if (notesBeingHit && hitResetTimer >= 0)
			{
				...
				if (hitResetTimer <= 0)
					notesBeingHit = false;
				if (missResetTimer > 0)
					missResetTimer -= 0.01 / (ClientPrefs.framerate / 60) * playbackRate;
			}
		...
		if (botEnergy > 0.2 && botEnergy < 1.8)
			energyBar.color = energyTxt.color = 0xFF0094FF;
		if (botEnergy < 0.2)
			energyBar.color = energyTxt.color = 0xFFC60000;
		if (botEnergy > 1.8)
			energyBar.color = energyTxt.color = 0xFF00BC12;
		...
		if (ClientPrefs.showNPS && (notesHitDateArray.length > 0 || oppNotesHitDateArray.length > 0))
		{
			...
			if (ClientPrefs.ratingCounter && judgeCountUpdateFrame <= 8 && judgementCounter != null)
				updateRatingCounter();
			if (scoreTxtUpdateFrame <= 8 && scoreTxt != null)
				updateScore();
		}
		...
		if (!opponentChart)
			displayedHealth = ClientPrefs.smoothHealth ? FlxMath.lerp(displayedHealth, health,
				0.1 / ((!ffmpegMode ? ClientPrefs.framerate : targetFPS) / 60)) : health;
		else
			displayedHealth = ClientPrefs.smoothHealth ? FlxMath.lerp(displayedHealth, maxHealth - health,
				0.1 / ((!ffmpegMode ? ClientPrefs.framerate : targetFPS) / 60)) : maxHealth
				- health;
		...
		{
			botplayUsed = true;
			if (botplayTxt.text == "this text is gonna kick you out of botplay in 10 seconds"
				|| botplayTxt.text == "Your Botplay Free Trial will end in 10 seconds.")
			{
			...
			if (SONG.event7 != null && SONG.event7 != "---" && SONG.event7 != '' && SONG.event7 != 'None')
				switch (SONG.event7)
				{
					...
					case "Game Over":
						health = 0;
					case "Go to Song":
						PlayState.SONG = Song.loadFromJson(SONG.event7Value
							+ (CoolUtil.difficultyString() == 'NORMAL' ? '' : '-' + CoolUtil.difficulties[storyDifficulty]), SONG.event7Value);
						LoadingState.loadAndSwitchState(PlayState.new);
					case "Close Game":
		...
		if (startedCountdown && !paused)
		{
			...
			if (!ffmpegMode)
			{
				...
				if (Conductor.songPosition < 200 && Math.abs(vocals.time - FlxG.sound.music.time) >= 20)
			...
			if (!paused)
			{
				if (updateTime && FlxG.game.ticks % (Std.int(ClientPrefs.framerate / 60) > 0 ? Std.int(ClientPrefs.framerate / 60) : 1) == 0)
				{
					...
					if (Conductor.songPosition - lastUpdateTime >= 1.0)
					{
						...
						if (ClientPrefs.timeBarType != 'Song Name')
						{
							timeTxt.text = ClientPrefs.timeBarType.contains('Time Left') ? CoolUtil.getSongDuration(Conductor.songPosition,
								songLength) : CoolUtil.formatTime(Conductor.songPosition)
						...
						if (cpuControlled && ClientPrefs.timeBarType != 'Song Name' && ClientPrefs.botWatermark)
							timeTxt.text += ' (Bot)';
					}
				}
				if (ffmpegMode)
		...
		{
			if (!ffmpegMode)
				renderedTxt.text = 'Rendered/Skipped: ${formatNumber(amountOfRenderedNotes)}/${formatNumber(skippedCount)}/${formatNumber(notes.members.length + sustainNotes.members.length)}/${formatNumber(maxSkipped)}';
			else
				renderedTxt.text = 'Rendered Notes: ${formatNumber(amountOfRenderedNotes)}/${formatNumber(maxRenderedNotes)}/${formatNumber(notes.members.length + sustainNotes.members.length)}';
		}
		...
		if (ffmpegMode)
		{
			...
			if (ClientPrefs.renderGCRate > 0 && (frameCaptured / targetFPS) % ClientPrefs.renderGCRate == 0)
				openfl.system.System.gc();
			frameCaptured++;
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, strumTime:Float)
	{
		...
		switch (eventName)
		{
			...
			case 'Enable Bot Energy':
				if (!cpuControlled)
				{
					...
					for (i in 0...varsFadeIn.length)
						FlxTween.tween(varsFadeIn[i], {alpha: 1}, 0.75, {ease: FlxEase.expoOut});
				}
			...
			case 'Camera Bopping':
				...
				if (_interval != 4)
					usingBopIntervalEvent = true;
				else
					usingBopIntervalEvent = false;
			...
			case 'Fake Song Length':
				...
				if (doTween = true)
					FlxTween.tween(this, {songLength: fakelength}, 1, {ease: FlxEase.expoOut});
				if (doTween = true && (Math.isNaN(fakelength)))
					FlxTween.tween(this, {songLength: FlxG.sound.music.length}, 1, {ease: FlxEase.expoOut});
			...
			case 'Windows Notification':
				if (value1 == "")
					value1 = "JS Engine";
				if (value2 == "")
					value2 = "Are you doing that one bambi song?";
					...
					switch (charType)
					{
						case 0:
							if (boyfriend.curCharacter != value2)
							{
								...
								if (!value2.startsWith('bf') && !value2.startsWith('boyfriend'))
									iconP1.changeIcon(boyfriend.healthIcon);
								else
								{
									if (ClientPrefs.bfIconStyle == 'VS Nonsense V2')
										iconP1.changeIcon('bfnonsense');
									if (ClientPrefs.bfIconStyle == 'Doki Doki+')
										iconP1.changeIcon('bfdoki');
									if (ClientPrefs.bfIconStyle == 'Leather Engine')
										iconP1.changeIcon('bfleather');
									if (ClientPrefs.bfIconStyle == "Mic'd Up")
										iconP1.changeIcon('bfmup');
									if (ClientPrefs.bfIconStyle == "FPS Plus")
										iconP1.changeIcon('bffps');
									if (ClientPrefs.bfIconStyle == "OS 'Engine'")
										iconP1.changeIcon('bfos');
								}
								if (boyfriend.noteskin != null)
									bfNoteskin = boyfriend.noteskin;
							}
						...
						case 1:
							if (dad.curCharacter != value2)
							{
								...
								var dadAnim:String = (dad.animation.curAnim != null
									&& dad.animation.curAnim.name.startsWith('sing') ? dad.animation.curAnim.name : '');
								...
								{
									if (botplayTxt != null)
										FlxTween.color(botplayTxt, 1, botplayTxt.color,
											FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]));

									if (scoreTxt != null && !ClientPrefs.hideHud)
										FlxTween.color(scoreTxt, 1, scoreTxt.color,
											FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]));
								}
								...
								if (dadAnim != '')
									dad.playAnim(dadAnim, true);
							}
							if (dad.noteskin != null)
								dadNoteskin = dad.noteskin;
	...
	public function loopSongLol()
	{
		...
		if (playbackRate >= TROLL_MAX_SPEED && ClientPrefs.trollMaxSpeed != 'Disabled')
		{ // Limit playback rate to the troll mode max speed
	...
	function judgeNote(note:Note = null, ?miss:Bool = false)
	{
		if (note == null)
			return;
		if (daRating == null)
			daRating = ratingsData[0]; // because it likes being stupid
		if (!cpuControlled)
		{
			...
			if (miss)
				daRating.image = 'miss';
			else if (ratingsData[0].image == 'miss')
				ratingsData[0].image = !ClientPrefs.noMarvJudge ? 'perfect' : 'sick';

			if (!miss)
			{
				if (!ClientPrefs.complexAccuracy)
	...
	private function popUpScore(note:Note = null, ?miss:Bool = false):Void
	{
		...
		if (popUpsFrame <= 3)
		{
			if (!ClientPrefs.comboStacking)
				while (popUpGroup.members.length > 0)
				{
			...
			if (showRating && ClientPrefs.ratingPopups && !ClientPrefs.simplePopups)
			{
				...
				if (!miss && ClientPrefs.colorRatingHit)
				{
					switch (daRating.name) // This is so stupid, but it works
					{
						...
						case 'good':
							rating.color = FlxColor.LIME;
						case 'bad':
							rating.color = FlxColor.ORANGE;
						case 'shit':
				...
				for (daLoop => i in separatedScore)
				{
					...
					if (ClientPrefs.colorRatingHit && !miss)
					{
						switch (daRating.name) // This is so stupid, but it works
						{
							...
							case 'good':
								numScore.color = FlxColor.LIME;
							case 'bad':
								numScore.color = FlxColor.ORANGE;
							case 'shit':
			...
			if (ClientPrefs.showMS && !ClientPrefs.hideHud)
			{
				...
				msTxt.y -= ClientPrefs.comboOffset[1];
				if (combo >= 10000)
					msTxt.x += 30 * (Std.string(combo).length - 4);
				FlxTween.tween(msTxt, {y: msTxt.y + 8}, 0.1 / playbackRate, {
					onComplete: function(_)
					{
				...
				if (miss)
					msTxt.color = FlxColor.fromRGB(204, 66, 66);
			}
			...
			if (ClientPrefs.ratingPopups && ClientPrefs.simplePopups && !ClientPrefs.hideHud)
			{
				...
				FlxTween.tween(judgeTxt.scale, {x: 1, y: 1}, 0.1 / playbackRate, {
					onComplete: function(_)
					{
			...
			if (ClientPrefs.ratingPopups && !ClientPrefs.simplePopups)
				popUpGroup.sort((o, a, b) ->
				{
				...
				if (plrInputNotes.length != 0)
				{
					...
					if (plrInputNotes.length > 1)
					{
						...
						if (doubleNote.noteData == funnyNote.noteData && doubleNote.doOppStuff == funnyNote.doOppStuff)
						{
							...
							goodNoteHit(doubleNote); // otherwise, hit doubleNote instead of killing it
					}
	...
	function noteMiss(daNote:Note = null, daNoteAlt:PreloadedChartNote = null):Void
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		if (daNote != null)
		{
			...
			if (SONG.needsVoices && !ffmpegMode)
				if (opponentChart && opponentVocals != null && opponentVocals.volume != 0)
					opponentVocals.volume = 0;
				else if (!opponentChart && vocals.volume != 0 || vocals.volume != 0)
					vocals.volume = 0;
			if (!practiceMode)
				songScore -= 10 * Std.int((opponentChart ? polyphonyOppo : polyphonyBF));

			totalPlayed++;
			if (missRecalcsPerFrame <= 3)
			...
			if (scoreTxtUpdateFrame <= 4 && scoreTxt != null)
				updateScore();
			if (ClientPrefs.ratingCounter && judgeCountUpdateFrame <= 4)
				updateRatingCounter();
			...
			callOnLuas('noteMiss', [
				(daNote.isSustainNote ? sustainNotes : notes)
				.members.indexOf(daNote),
				daNote.noteData,
				daNote.noteType,
				daNote.isSustainNote
			]);
			if (ClientPrefs.missRating && ClientPrefs.ratingPopups)
			...
			if (ClientPrefs.showNotes || !ClientPrefs.showNotes && !cpuControlled)
			{
				while (targetNote.strumTime - Conductor.songPosition < (NOTE_SPAWN_TIME / targetNote.multSpeed))
				{
					if (ClientPrefs.fastNoteSpawn)
						(targetNote.isSustainNote ? sustainNotes : notes).spawnNote(targetNote);
					else
					{
					...
					if (!ClientPrefs.noSpawnFunc)
						callOnLuas('onSpawnNote', [
							(!spawnedNote.isSustainNote ? notes.members.indexOf(spawnedNote) : sustainNotes.members.indexOf(spawnedNote)),
							targetNote.noteData,
							targetNote.noteType,
	...
	function updateNote(daNote:Note):Void
	{
		if (daNote != null && daNote.exists)
		{
			...
			if (daNote.mustPress)
			{
				if ((cpuControlled || usingBotEnergy && strumsHeld[daNote.noteData])
					&& !daNote.wasGoodHit
			...
			if (maxRenderedNotes < amountOfRenderedNotes)
				maxRenderedNotes = amountOfRenderedNotes;
			...
			if (daNote.isSustainNote)
			{
				...
				if (strum != null && strum.sustainReduce)
			...
			{
				if (daNote.mustPress
					&& (!(cpuControlled || usingBotEnergy && strumsHeld[daNote.noteData]) || cpuControlled)
					&& !daNote.ignoreNote
					&& !endingSong
					&& !daNote.wasGoodHit)
				{
	...
	function goodNoteHit(note:Note, noteAlt:PreloadedChartNote = null):Void
	{
		if (note != null)
		{
			...
			if (!ffmpegMode && (note.wasGoodHit || cpuControlled && note.ignoreNote))
				return;
			...
			if (ClientPrefs.hitsoundVolume > 0 && !note.hitsoundDisabled && !note.isSustainNote)
			{
				hitsound.play(true);
				hitsound.pitch = playbackRate;
				if (FileSystem.exists('assets/shared/images/' + hitsoundImageToLoad + '.png')
					|| FileSystem.exists(Paths.modFolders('images/' + hitsoundImageToLoad + '.png'))
					&& hitImagesFrame < 4)
				{
					hitImagesFrame++;
			...
			if (!note.hitCausesMiss)
			{
				if (!note.isSustainNote)
				{
					if (combo < 0)
						combo = 0;
					if ((opponentChart ? polyphonyOppo : polyphonyBF) > 1 && !note.isSustainNote)
						totalNotes += polyphonyBF - 1;
					missCombo = 0;
					combo += 1 * (opponentChart ? polyphonyOppo : polyphonyBF);
					totalNotesPlayed += 1 * (opponentChart ? polyphonyOppo : polyphonyBF);
					if (ClientPrefs.showNPS)
					{ // i dont think we should be pushing to 2 arrays at the same time but oh well
				...
				if (!usingBotEnergy)
					health += note.hitHealth * healthGain * (opponentChart ? polyphonyOppo : polyphonyBF);

				if (bothSides)
					oppTrigger = bothSides && note.doOppStuff;
				else if (opponentChart && !oppTrigger)
					oppTrigger = true;
				doGf = note.gfNote;
				...
				if (!note.noAnimation && ClientPrefs.charsAndBG)
				{
					...
					if (ClientPrefs.cameraPanning)
						camPanRoutine(animToPlay, (!oppTrigger ? 'bf' : 'oppt'));
					if (playerChar != null)
					{
						...
						if (note.isSustainNote)
						{
							...
							if (playerChar.animation.exists(holdAnim))
								animToPlay = holdAnim;
						...
						if (canPlay)
							playerChar.playAnim(animToPlay, true);
				...
				if ((cpuControlled || usingBotEnergy && strumsHeld[note.noteData])
					&& ClientPrefs.botLightStrum
					&& !strumsHit[(note.noteData % 4) + 4])
				{
			...
			if (!ClientPrefs.lessBotLag && ClientPrefs.noteSplashes && note.isSustainNote && splashesPerFrame[3] <= 4)
				spawnHoldSplashOnNote(note);
			if (SONG.needsVoices && !ffmpegMode)
				if (opponentChart && opponentVocals != null && opponentVocals.volume != 1)
					opponentVocals.volume = 1;
				else if (!opponentChart && vocals.volume != 1 || vocals.volume != 1)
					vocals.volume = 1;
			...
			{
				callOnLuas((oppTrigger ? 'opponentNoteHit' : 'goodNoteHit'), [
					(note.isSustainNote ? sustainNotes : notes)
					.members.indexOf(note),
			...
			if (ClientPrefs.ratingCounter && judgeCountUpdateFrame <= 4)
				updateRatingCounter();
			if (scoreTxtUpdateFrame <= 4)
				updateScore();
			if (ClientPrefs.iconBopWhen == 'Every Note Hit'
				&& (iconBopsThisFrame <= 2 || ClientPrefs.noBopLimit)
				&& !note.isSustainNote
				&& iconP1.visible)
				bopIcons(!oppTrigger);
			return;
		...
		if (noteAlt != null)
		{
			...
			if (!noteAlt.isSustainNote && cpuControlled)
			{
				...
				if ((opponentChart ? polyphonyOppo : polyphonyBF) > 1)
					totalNotes += (opponentChart ? polyphonyOppo : polyphonyBF) - 1;
			}
			if (!ClientPrefs.noSkipFuncs)
				callOnLuas((oppTrigger ? 'opponentNoteSkip' : 'goodNoteSkip'), [null, Math.abs(noteAlt.noteData), noteAlt.noteType, noteAlt.isSustainNote]);
	...
	function opponentNoteHit(daNote:Note, noteAlt:PreloadedChartNote = null):Void
	{
		if (daNote != null)
		{
			...
			else if (!daNote.noAnimation && ClientPrefs.charsAndBG)
			{
				...
				if (ClientPrefs.cameraPanning)
					camPanRoutine(animToPlay, (!opponentChart ? 'dad' : 'bf'));
				...
				if (oppChar != null)
				{
					...
					{
						holdAnim = animToPlay + '-hold';
						if (oppChar.animation.exists(holdAnim))
							animToPlay = holdAnim;
			...
			if (polyphonyOppo > 1 && !daNote.isSustainNote)
				opponentNoteTotal += polyphonyOppo - 1;
			...
			if (ClientPrefs.oppNoteSplashes && daNote.isSustainNote && splashesPerFrame[2] <= 4)
				spawnHoldSplashOnNote(daNote, true);

			if (!ClientPrefs.noHitFuncs)
			{
				callOnLuas((!opponentChart ? 'opponentNoteHit' : 'goodNoteHit'), [
					(daNote.isSustainNote ? sustainNotes : notes)
					.members.indexOf(daNote),
		...
		if (noteAlt != null)
		{
			...
			if ((!noteAlt.gfNote ? !opponentChart ? dad : boyfriend : gf) != null
				&& (!noteAlt.gfNote ? !opponentChart ? dad : boyfriend : gf).shakeScreen)
			{
				oppChar = !noteAlt.gfNote ? !opponentChart ? dad : boyfriend : gf;
				camGame.shake(oppChar.shakeIntensity, oppChar.shakeDuration / playbackRate);
				camHUD.shake(oppChar.shakeIntensity / 2, oppChar.shakeDuration / playbackRate);
			...
			if (!ClientPrefs.noSkipFuncs)
				callOnLuas((!opponentChart ? 'opponentNoteSkip' : 'goodNoteSkip'), [null, Math.abs(noteAlt.noteData), noteAlt.noteType, noteAlt.isSustainNote]);
			if (SONG.needsVoices && !ffmpegMode)
				if (!opponentChart && opponentVocals != null && opponentVocals.volume != 1)
					opponentVocals.volume = 1;
				else if (opponentChart && vocals.volume != 1 || vocals.volume != 1)
	...
	public function playerDance():Void
	{
		...
		if (char.holdTimer > Conductor.stepCrochet * (0.0011 #if FLX_PITCH / FlxG.sound.music.pitch #end) * char.singDuration * singDurMult
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			if (ClientPrefs.timeBarStyle == 'Leather Engine')
				timeBar.color = SONG.notes[curSection].mustHitSection ? FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1],
					boyfriend.healthColorArray[2]) : FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
		...
		else if (ClientPrefs.iconBopWhen == 'Every Note Hit')
		{
			...
			if (ClientPrefs.iconBounceType == 'Old Psych')
			{
				if (bopBF)
					...
					iconP2.setGraphicSize(Std.int(iconP2.width + 30), Std.int(iconP2.height + 30));
			}
			if (ClientPrefs.iconBounceType == 'Strident Crisis')
			{
				final funny:Float = (healthBar.percent * 0.01) + 0.01;
	...
	public function callOnLuas(event:String, args:Array<Dynamic> = null, ignoreStops = true, exclusions:Array<String> = null,
	...
	public function updateRatingCounter()
	{
		...
		final hittingStuff = (!ClientPrefs.lessBotLag
			&& ClientPrefs.showComboInfo
			&& !cpuControlled ? 'Combo: $formattedCombo/${formattedMaxCombo}\n' : '')
			+ 'Hits: '
			+ formatNumber(totalNotesPlayed)
			+ ' / '
			+ formatNumber(totalNotes)
			+ ' ('
			+ FlxMath.roundDecimal((totalNotesPlayed / totalNotes) * 100, 2)
			+ '%)';
		final ratingCountString = (!cpuControlled
			|| cpuControlled
			&& !ClientPrefs.lessBotLag ? '\n'
		...
		if (ClientPrefs.opponentRateCount)
			judgementCounter.text += '\n\nOpponent Hits: '
				+ formattedEnemyHits
				+ ' / '
				+ formatNumber(opponentNoteTotal)
				+ ' ('
				+ FlxMath.roundDecimal((enemyHits / opponentNoteTotal) * 100, 2)
				+ '%)'
		...
		for (name in achievesToCheck)
		{
			...
			else // any FC achievements, name should be "weekFileName_nomiss", e.g: "week3_nomiss";
			{
				if (isStoryMode
					...
					&& CoolUtil.currentDifficulty.toUpperCase() == 'HARD'
	...
	private function initRender():Void
	{
		...
		if (!FileSystem.exists('assets/gameRenders/'))
		{ // In case you delete the gameRenders folder
			trace('gameRenders folder not found! Creating the gameRenders folder...');
			FileSystem.createDirectory('assets/gameRenders');
		}
		else if (!FileSystem.isDirectory('assets/gameRenders/'))
		{
	...
	private function pipeFrame():Void
	{
		...
		img = lime.app.Application.current.window.readPixels(new lime.math.Rectangle(FlxG.scaleMode.offset.x, FlxG.scaleMode.offset.y,
			FlxG.scaleMode.gameSize.x,
