...
class GameplaySettingsSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('Auto-Pause if Focus Lost', "If unchecked, the game won't pause when your game loses focus.", 'autoPause', 'bool', true);
		addOption(option);

		var option:Option = new Option('Complex Accuracy', "If checked, accuracy is MS-based instead of rating-based.", 'complexAccuracy', 'bool', false);
		addOption(option);

		var option:Option = new Option('Remove Perfect!! Judgement', "If checked, removes the Perfect judgement.", 'noMarvJudge', 'bool', false);
		addOption(option);
		...
		var option:Option = new Option('Instant Respawn', "Instantly respawn when you die.", 'instaRestart', 'bool', false);
		addOption(option);

		var option:Option = new Option('More Spammable Inputs', 'If checked, the input system is more spammable.', 'ezSpam', 'bool', false);
		addOption(option);

		var option:Option = new Option('Shit Gives Miss', "If checked, hitting a Shit rating will count as a miss.", 'shitGivesMiss', 'bool', false);
		addOption(option);

		var option:Option = new Option('SGM Intensity:', "How intense would you want the threshold for missing from a rating to be?", 'ratingIntensity',
			'string', 'Normal', ['Normal', 'Harsh', 'Very Harsh']);
		addOption(option);

		var option:Option = new Option('Enable Taunt Key', "If checked, pressing the Taunt key will make BF go HEY!!", 'spaceVPose', 'bool', true);
		addOption(option);

		var option:Option = new Option('Ghost Tapping Plays Anim', "If checked, Ghost Tapping will play BF's animations.", 'ghostTapAnim', 'bool', true);
		addOption(option);

		var option:Option = new Option('Enable Miss Sound', "If checked, re-enables the miss sound when you miss a note.", 'missSoundShit', 'bool', false);
		addOption(option);
		...
		if (hitsoundList.length > 0)
		{
			...
			var option:Option = new Option('Hitsound:', "What type of hitsound would you like?", 'hitsoundType', 'string', 'osu!mania', hitsoundList);
			addOption(option);
		}
		...
		if (ClientPrefs.safeFrames > 10)
			option.displayFormat = '%v (Will not save)';
		option.changeValue = 0.1;
		addOption(option);

		var option:Option = new Option('Voiid-Styled Troll Mode', "If checked, Troll Mode's speed change will behave like in Voiid Chronicles 1.5/2.0",
			'voiidTrollMode', 'bool', false);
		addOption(option);
		...
		var option:Option = new Option('Troll Mode Speed Limit:',
			...
			'trollMaxSpeed', 'string', 'Medium', ['Disabled', 'Lowest', 'Lower', 'Low', 'Medium', 'High', 'Highest']);
		addOption(option);
