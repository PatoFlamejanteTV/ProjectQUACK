...
class SuperSecretDebugMenu extends BaseOptionsMenu
{
	public function new()
	{
		...
		FlxTween.tween(FlxG.cameras.list[FlxG.cameras.list.length - 1], {zoom: 1, alpha: 1}, 2, {
		...
		var option:Option = new Option('No Ascend RNG', "If checked, makes the RNG Guns ascend part play every time.", 'noGunsRNG', 'bool', false);
		addOption(option);

		var option:Option = new Option('Playback Rate Controls',
			"If checked, allows you to control the playback rate in PlayState \nwithout the need for the Pause Menu.", 'pbRControls', 'bool', false);
		addOption(option);

		var option:Option = new Option('No Wheatley Space RNG', "If checked, makes the RNG Wheatley Space easter egg play every time.", 'wheatleySpace',
			'bool', false);
		addOption(option);

		var option:Option = new Option('Crash the Engine', "Select this to crash the engine.", 'crashEngine', 'link', false);

		option.onChange = crashDaEngine;
		addOption(option);

		var option:Option = new Option('Test Update Screen', "Select this to test the engine's update screen.", 'updateThing', 'link', false);

		option.onChange = doDaUpdate;
		addOption(option);

		var option:Option = new Option('Rainbow FPS', "If checked, allows the FPS color to be rainbow.", 'rainbowFPS', 'bool', false);
		addOption(option);

		var option:Option = new Option('Force Off GC while Rendering', "If checked, the game will force GC off while rendering. Beware of memory leaks!",
			'noRenderGC', 'bool', false);
		addOption(option);
	...
	function crashDaEngine():Void
	{
		...
		nextMessage = function()
		{
			...
			{
				openSubState(new Prompt(messages[i], 0, function()
				{
					nextMessage();
				}, null, false, "Yes", "No"));
			}
