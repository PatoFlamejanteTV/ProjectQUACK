...
class NotesSubState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
		...
		{
			var colors:Array<FlxColor> = ClientPrefs.noteColorStyle != 'Quant-Based' ? !onPixel ? ClientPrefs.defaultArrowRGB[curSelectedNote] : ClientPrefs.defaultPixelRGB[curSelectedNote] : ClientPrefs.defaultQuantRGB[curSelectedNote];
	...
	public function spawnNotes()
	{
		...
		if (onPixel && !Paths.fileExists('images/pixelUI/' + Paths.defaultSkin + '.png', IMAGE))
		{
			CoolUtil.coolError("HEY! Your Noteskin doesn't have any Pixel sprites. The game will revert to non-pixel notes to prevent a crash."
				+ "\n\nIf it DOES have Pixel sprites, make sure they're located in 'images/pixelUI/noteskins/'.",
		...
		if (onPixel)
			@:privateAccess bigNote.reloadNote(Paths.defaultSkin);
		else
			bigNote.texture = Paths.defaultSkin;
