...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		if (_song.arrowSkin != null && _song.arrowSkin.length > 1)
			skin = _song.arrowSkin;
	...
	function addSectionUI():Void
	{
		...
		var copyLastButton:FlxButton = new FlxButton(10, swapSection.y + 30, "Copy last section", function()
		{
			...
			if (_song.notes[daSec - value] == null || _song.notes[daSec] == null)
				return;
		...
		{
			if (_song.notes[curSection] == null || _song.notes[curSection] != null && _song.notes[curSection].sectionNotes == null)
				return;
		...
		copyMultiSectButton = new FlxButton(CopyFutureSectionCount.x, CopyLastSectionCount.y
			+ 40,
			"Copy from the last "
			+ Std.int(CopyFutureSectionCount.value)
			+ " to the next "
			+ Std.int(CopyFutureSectionCount.value)
			+ " sections, "
			+ Std.int(CopyLoopCount.value)
			+ " times",
			function()
			{
		...
		var copyNextButton:FlxButton = new FlxButton(CopyNextSectionCount.x, CopyNextSectionCount.y + 20, "Copy to the next..", function()
		{
			...
			if (_song.notes[curSec] == null
				|| _song.notes[curSec] != null
				&& _song.notes[curSec].sectionNotes.length < 1
				|| Math.isNaN(_song.notes[curSec].sectionNotes.length)
				|| _song.notes[curSec].sectionNotes == null)
			{
				trace("HEY! your section doesn't have any notes! please place at least 1 note then try using this.");
				return; // prevent a crash if the section doesn't have any notes
			}
		...
		deleteSections = new FlxButton(deleteSecStart.x
			+ 30, CopyLastSectionCount.y
			+ 40,
			"Delete sections "
			+ Std.int(deleteSecStart.value)
			+ " to "
			+ Std.int(deleteSecEnd.value), function()
		{
				...
				if (_song.notes[startSec + i] != null && _song.notes[startSec + i].sectionNotes != null)
					...
					else
					{
						...
						while (b >= 0)
						{
							...
							if (note != null
								&& deleteBfNotes
								&& (note[1] < 4 ? _song.notes[startSec + i].mustHitSection : !_song.notes[startSec + i].mustHitSection))
								_song.notes[startSec + i].sectionNotes.remove(note);
							if (note != null
								&& deleteOppNotes
								&& (note[1] < 4 ? !_song.notes[startSec + i].mustHitSection : _song.notes[startSec + i].mustHitSection))
								_song.notes[startSec + i].sectionNotes.remove(note);
	...
	function addChartingUI()
	{
		...
		if (FlxG.save.data.allowUndo == null)
			FlxG.save.data.allowUndo = true;
		...
		if (FlxG.save.data.showGrid == null)
			FlxG.save.data.showGrid = false;
		...
		if (FlxG.save.data.chart_hitsoundVolume == null)
			FlxG.save.data.chart_hitsoundVolume = 1;
		...
		if (FlxG.save.data.soundEffects == null)
			FlxG.save.data.soundEffects = true;
		...
		if (FlxG.save.data.idleMusicAllowed == null)
			FlxG.save.data.idleMusicAllowed = true;
		...
		idleMusicCheck.callback = function()
		{
			...
			if (!FlxG.sound.music.playing)
			{
				if (idleMusicAllow)
				{
					if (!idleMusic.musicPaused)
						idleMusic.playMusic();
					else
						idleMusic.unpauseMusic(0.3);
				}
				else
					idleMusic.pauseMusic();
			}
	...
	override function update(elapsed:Float)
	{
		...
		if (idleMusic != null && idleMusic.music != null && idleMusic.music.playing && !idleMusicAllow)
			idleMusic.pauseMusic();
			...
			&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * getSectionBeats() * 4) * zoomList[curZoom])
		{
			...
			if (selectionNote.animation.curAnim == null)
				selectionNote.playAnim('static' + selectionNote.noteData, false);
			else if (!selectionNote.animation.curAnim.name.endsWith(Std.string(selectionNote.noteData)))
				selectionNote.playAnim('static' + selectionNote.noteData, false);
		}
		else
		{
	...
	function reloadGridLayer()
	{
		...
		if (sectionStartTime(1) <= FlxG.sound.music.length)
		{
			if (showTheGrid)
			{
				...
				nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9,
					Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
	...
	function setupSusNote(note:Note, beats:Float):FlxSprite
	{
		...
		if (note.noteType == "Hurt Note")
			color = CoolUtil.dominantColor(note); // Make black if hurt note
...
class SelectionNote extends FlxSprite
{
	...
	{
		if (rgbShader == null || rgbShader != null && !rgbShader.enabled)
			return;
