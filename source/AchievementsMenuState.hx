...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		progressBar = new FlxBar(progressBarBG.x + 4, progressBarBG.y + 4, LEFT_TO_RIGHT, Std.int(progressBarBG.width - 8), Std.int(progressBarBG.height - 8),
			this, 'progressValue');
			...
			if (add != 0)
			{
				...
				if (curRow != oldRow)
				{
					if (curRow < oldRow)
						...
						curSelected = curSelected -= rowSize;
				}
				_changeSelection();
			}
	...
	function _changeSelection()
	{
		...
		if (hasProgress)
		{
			...
			barTween = FlxTween.num(progressValue, (val1 / val2) * 100, 0.5, {
		...
		if (controls.ACCEPT)
		{
			if (onYes)
			{
				...
				if (option.maxProgress > 0)
					state.progressTxt.text = '0 / ' + option.maxProgress;
				...
				if (state.progressBar.visible)
				{
					...
					state.barTween = FlxTween.num(state.progressValue, 0, 0.5, {
						ease: FlxEase.quadOut,
						onUpdate: function(twn:FlxTween)
						{
