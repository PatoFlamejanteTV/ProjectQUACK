...
class Note extends FlxSprite
{
	...
	{
		if (value.length == 0)
			value = Paths.defaultSkin;
		if (!pixelNote && texture != value)
		{
			changeSize = false;
			if (!Paths.noteSkinFramesMap.exists(value))
				Paths.initNote(value);
			if (frames != @:privateAccess Paths.noteSkinFramesMap.get(value))
				frames = @:privateAccess Paths.noteSkinFramesMap.get(value);
			if (animation != @:privateAccess Paths.noteSkinAnimsMap.get(value))
	...
	private function set_noteType(value:String):String
	{
		...
		if (ClientPrefs.noteColorStyle == 'Normal' && rgbShader != null && useRGBShader)
			defaultRGB();
	...
	public function new(?newStrumTime:Float, ?newNoteData:Int)
	{
		...
		if (!Math.isNaN(newNoteData) && pixelNote)
			noteData = newNoteData;
		if (!Math.isNaN(newStrumTime))
	...
	{
		if (Type.getClassName(Type.getClass(FlxG.state)) == 'PlayState' && PlayState.instance.cpuControlled)
			return;
	...
	inline public function followStrum(strum:StrumNote, songSpeed:Float = 1):Void
	{
		...
		if (copyY)
		{
			y = strum.y
				+ offsetY
				+ (!isSustainNote || ClientPrefs.downScroll ? 0 : 55)
				+ Math.sin(strum.direction * Math.PI / 180) * distance;
	...
	public function clipToStrumNote(myStrum:StrumNote)
	{
		...
		if (isSustainNote && (mustPress || !ignoreNote) && (!mustPress || (wasGoodHit || !canBeHit)))
		{
			...
			switch (ClientPrefs.noteColorStyle)
			{
				...
				case 'Char-Based':
					if (PlayState.instance != null)
					{
						...
						if (gfNote)
							arr = CoolUtil.getHealthColors(PlayState.instance.gf);
						if (noteData > -1)
						{
	...
	public function setupNoteData(chartNoteData:PreloadedChartNote):Void
	{
		...
		else
		{
			if (!visible)
				visible = true;
			if (alpha != 1)
				alpha = 1; // if (multAlpha != 1) multAlpha = 1;
		}
		if (flipY)
			flipY = false;
	}
